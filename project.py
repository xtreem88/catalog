...
def restaurantXML():
    ...
    sitemap_xml = render_template('restaurant_feed.xml', items=items)
...
def restaurantMenuXML(restaurant_id):
    ...
    items = session.query(MenuItem).filter_by(
        restaurant_id=restaurant_id).all()
    sitemap_xml = render_template('menu_feed.xml', items=items)
    response = Response(sitemap_xml)
    response.headers["Content-Type"] = "application/xml"
...
def menuItemXML(restaurant_id, menu_id):
    ...
    sitemap_xml = render_template('menuitem_feed.xml', item=menuItem)
    response = Response(sitemap_xml)
    response.headers["Content-Type"] = "application/xml"
...
def editRestaurant(restaurant_id):
    ...
    if editedRestaurant.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You\
         are not authorized to edit this restaurant.\
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            filename = secure_filename(file.filename)
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
            editedRestaurant.picture = filename
...
def deleteRestaurant(restaurant_id):
    ...
    if restaurantToDelete.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are\
         not authorized to delete this restaurant. Please\
...
def newMenuItem(restaurant_id):
    ...
    if login_session['user_id'] != restaurant.user_id:
        return "<script>function myFunction() {alert('You\
         are not authorized to add menu items to this\
        ...
        return "<script>function myFunction() {alert(\
          'You are not authorized to delete menu items\
           to this restaurant. Please create your own restaurant\
            in order to delete items.');}</script><body\
             onload='myFunction()''>"
    if request.method == 'POST':
        token = login_session.pop('_csrf_token', None)
        if not token or token != request.form.get('_csrf_token'):
